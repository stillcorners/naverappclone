...
### 3. ES 모듈 환경에서 CommonJS 전역 변수인 dirname을 사용하는 방법
...
### 5. `git add` 할 때에 터미널에서 LF will be replaced by CRLF in.. 에러 해결 방법
...
### 6. 원활한 배포를 위해 상대 경로로 작성하는 것이 좋다
...
### 7. 깃허브 페이지 배포는 절대 경로가 권장되었지만, Heroku 배포는 상대 경로가 권장된다.
...
### 11. CSS 속성과 SCSS 믹스인을 활용한 스프라이트 이미지 추출하는 방법
